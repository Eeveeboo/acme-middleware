...
export async function loadCert(servername: string, production:boolean, email?: string): Promise<tls.SecureContext> {
    ...
    if (!exists) {
       ...
       await createCert({ production, domain, email, altNames });
    }
